# Hi there, I'm Vivek Tiwari.

### building systems that think & execute.

AI & Automation at **Boom Games, Bengaluru**. Studying AI in Business at **ISB, Hyderabad**. Exploring agentic workflows, multi-agent systems, and the space between AI and reliability.

<a href="https://chaiovercode.com" target="_blank"><strong>chaiovercode.com</strong></a>

---

### What I'm Up To

- Mastering multi-agent patterns with **LangGraph**, **CrewAI** & **AutoGen**
- Building collaborative systems with **MCP** (Model Context Protocol)
- Currently reading *Prisoners of Geography* by Tim Marshall

---

### Featured Projects

#### [OnAir](https://chaiovercode.com/onair)
> **Native macOS menu bar app** that keeps your meetings always visible. Focus timer, auto-join, conflict detection, natural language event creation, stats dashboard. 2MB, no Electron.
>
> *Stack:* Swift, SwiftUI, EventKit

#### [OverGraph](https://overgraph.in)
> **IPL match visualizer** with Elo ratings, live win-probability from 10K Monte Carlo simulations per over, and over-by-over scoring breakdowns.
>
> *Stack:* React, TypeScript, Monte Carlo, Elo

#### [Narrativ](https://chaiovercode.com/narrativ)
> **AI-powered visual storytelling app.** Research any topic, generate beautiful image carousels, and export for social media. Local-first with iCloud sync.
>
> *Stack:* Tauri, React, Python, Gemini API

#### [GapTrack](https://chaiovercode.github.io/gaptrack/#/)
> **Personal progress tracker** that helps you bridge the gap between where you are and where you want to be. Track goals, habits, and growth metrics with beautiful visualizations.
>
> *Stack:* React, TypeScript, Tailwind, Charts

#### [AnnotateThis](https://github.com/chaiovercode/AnnotateThis)
> **PDF annotation tool powered by Gemini.** Upload a document and it auto-highlights key insights. Making dense PDFs easier to digest.
>
> *Stack:* TypeScript, Gemini API, PDF.js

#### [Stock Arena](https://github.com/chaiovercode/stockarena)
> **Stock analysis using multiple AI agents.** Three agents debate — bull, bear, and judge — to analyze stocks from different angles. Exploring CrewAI patterns.
>
> *Stack:* CrewAI, LangGraph, Python

#### [AQI Map](https://github.com/chaiovercode/ai-engineering/blob/main/apps/aqi-cards/aqi.py)
> **Generative art engine for air quality.** Paints your city's air quality in real-time using satellite data to render 3D miniatures — smoggy for Delhi, clear for Bangalore.
>
> *Stack:* Python, Gemini

#### [AI Leadership Notes](https://chaiovercode.github.io/ai-leadership-notes/)
> **Open-source curriculum** bridging the gap between technical AI foundations and business strategy. Frameworks, case studies, and notes on leading AI initiatives.
>
> *Stack:* HTML, GitHub Actions, Claude Code

---

### Tech Stack

**AI & LLMs**
<br>
<img src="https://img.shields.io/badge/LangGraph-1C2C50?style=flat&logo=mediafire&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/CrewAI-000000?style=flat&logo=robot&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/RAG-009688?style=flat&logo=bookstack&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/MCP-4A154B?style=flat&logo=serverless&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Hugging_Face-FFD21E?style=flat&logo=huggingface&logoColor=black" height="30"/>

**Automation**
<br>
<img src="https://img.shields.io/badge/n8n-FF6560?style=flat&logo=n8n&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white" height="30"/>

**Frontend & Apps**
<br>
<img src="https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black" height="30"/>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Swift-F05138?style=flat&logo=swift&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Tauri-24C8D8?style=flat&logo=tauri&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Vite-646CFF?style=flat&logo=vite&logoColor=white" height="30"/>

**Data & Viz**
<br>
<a href="https://public.tableau.com/app/profile/chaiovercode/vizzes" target="_blank"><img src="https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white" height="30"/></a>
<img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Gradio-FF4B4B?style=flat&logo=gradio&logoColor=white" height="30"/>

**Dev & Core**
<br>
<img src="https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white" height="30"/>
<img src="https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white" height="30"/>

---

### Work Highlights

- **RAG Knowledge Hub** — Built a retrieval-augmented system for internal queries, cutting search time across Slack and docs.
- **AI Call Quality System** — S3 + ElevenLabs + Deepseek pipeline that transcribes and scores calls at 90%+ accuracy vs human reviewers.
- **24/7 Community Automation** — n8n + Python agents handling Instagram & Telegram auto-replies with Google Sheets logging, zero human intervention.
- **Self-Serve Data Platform** — Streamlit on Databricks app for non-technical teams; built an internal MCP for multi-API user lookups.
- **Expected Goals (xG) Model** — Football highlight detection model that cut editing time from 1 hour to 10 minutes per match.
- **Player Analytics** — Valuation and recommendation systems for IPL and Pro Kabaddi franchises, reaching 100M+ users.

---

<p align="left">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=chaiovercode&theme=tokyonight" alt="chaiovercode's Stats" />
</p>

---

### Let's Connect

<p align="left">
<a href="https://chaiovercode.com" target="_blank"><img src="https://img.shields.io/badge/Portfolio-000000?style=flat&logo=vercel&logoColor=white" height="30"/></a> <a href="https://x.com/chaiovercode" target="_blank"><img src="https://img.shields.io/badge/X-000000?style=flat&logo=x&logoColor=white" height="30"/></a> <a href="https://www.linkedin.com/in/vivektiwari13/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white" height="30"/></a> <a href="https://instagram.com/chaiovercode" target="_blank"><img src="https://img.shields.io/badge/Instagram-E4405F?style=flat&logo=instagram&logoColor=white" height="30"/></a> <a href="https://chaiovercode.substack.com/" target="_blank"><img src="https://img.shields.io/badge/Substack-FF6719?style=flat&logo=substack&logoColor=white" height="30"/></a> <a href="https://public.tableau.com/app/profile/chaiovercode/vizzes" target="_blank"><img src="https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white" height="30"/></a> <a href="https://huggingface.co/chaiovercode" target="_blank"><img src="https://img.shields.io/badge/Hugging_Face-FFD21E?style=flat&logo=huggingface&logoColor=black" height="30"/></a> <a href="mailto:chaiovercode@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white" height="30"/></a>
</p>

